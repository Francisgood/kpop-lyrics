...
describe("shared auth policy", () => {
  it("is disabled by default and rejects partial or non-origin config", () => {
    expect(sharedAuthEnabled({})).toBe(false);
    expect(getSharedAuthConfig({})).toBeNull();
    expect(
      getSharedAuthConfig({
        ...env,
        AEGYO_APP_ORIGIN: "https://aegyo.example.test/callback",
      }),
    ).toBeNull();
    expect(
      getSharedAuthConfig({ ...env, AEGYO_AUTH_BASE_URL: "ftp://localhost" }),
    ).toBeNull();
  });
  it("uses the Accounts issuer and a 30 second absolute state bound", () => {
    expect(getSharedAuthConfig(env)).toMatchObject({
  ...
  it("rejects inactive, mismatched, malformed, and stale provider state", () => {
    ...
    expect(
      parseProviderSecurityState({ ...state, securityVersion: "4" }),
    ).toBeNull();
    const session = {
      user: { sharedIdentity: { subject: "sub-1" } },
      providerSessionId: "sid-1",
    ...
    expect(
      stateAllowsSession(session, state, Date.parse("2026-09-11T12:00:01Z")),
    ).toBe(true);
    expect(
      stateAllowsSession(
        session,
        { ...state, active: false },
        Date.parse("2026-09-11T12:00:01Z"),
      ),
    ).toBe(false);
    expect(
      stateAllowsSession(
        session,
        { ...state, securityVersion: 5 },
        Date.parse("2026-09-11T12:00:01Z"),
      ),
    ).toBe(false);
    expect(
      stateAllowsSession(
        session,
        {
          ...
          passwordResetState: {
            ...reset,
            lastPasswordReset: "2026-09-11T11:00:00.000Z",
          },
        },
        Date.parse("2026-09-11T12:00:01Z"),
      ),
  ...
  it("permits only one bounded fresh-auth retry after reset", () => {
    const common = {
      authTime: Date.parse("2026-09-11T12:00:00Z") / 1000,
      resetState: { ...reset, lastPasswordReset: "2026-09-11T12:00:00.500Z" },
    ...
    ).toMatchObject({ kind: "deny", reason: "reauthentication_failed" });
  });
});
