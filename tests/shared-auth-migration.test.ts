...
describe("additive shared-auth migration", () => {
  it("keeps User IDs and owned data untouched while adding an explicit one-to-one mapping", () => {
    ...
    expect(sql).toContain("CHECK (\"id\" = 'accounts-shared-auth-v1')");
    expect(sql).toContain("CHECK (\"mappingDigest\" ~ '^[0-9a-f]{64}$')");
    expect(sql).toContain('BEFORE UPDATE OR DELETE ON "AuthCutoverLatch"');
    expect(sql).toContain('BEFORE TRUNCATE ON "AuthCutoverLatch"');
    expect(sql).toContain('UNIQUE INDEX "SharedAuthIdentity_userId_key"');
    expect(sql).toContain(
      'UNIQUE INDEX "SharedAuthIdentity_issuer_subject_key"',
    );
  ...
  it("adds only nullable security metadata to legacy sessions", () => {
    ...
    expect(sql).not.toMatch(
      /ADD COLUMN "(?:providerSessionId|authenticatedAt|providerCheckedAt|securityVersion|passwordResetAt)"[^,;]*NOT NULL/,
    );
  });
});
