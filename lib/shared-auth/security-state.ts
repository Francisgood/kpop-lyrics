...
export function stateAllowsSession(
  session: {
    user: { sharedIdentity: { subject: string } | null };
  ...
  const currentResetMs =
    state.passwordResetState.lastPasswordReset === null
      ? null
      : parseResetInstant(state.passwordResetState.lastPasswordReset);
  if (
    ...
    !state.active ||
    state.subject !== session.user.sharedIdentity.subject ||
    state.providerSessionId !== session.providerSessionId ||
    state.securityVersion !== session.securityVersion
  )
    return false;
  if ((session.passwordResetAt?.getTime() ?? null) !== currentResetMs)
    return false;
  return (
    evaluateFreshness({
