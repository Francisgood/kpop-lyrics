...
// PII (name/email/phone/zip/birthDate) — keep IMAGE_REFRESH_SECRET private.
...
type Row = {
  ...
  birthDate: Date; newsletterOptIn: boolean; referralCode: string;
...
export async function GET(req: NextRequest) {
  ...
  try {
    rows = await prisma.$queryRawUnsafe<Row[]>(
      `SELECT "firstName","lastName","email","phone","zip","birthDate","newsletterOptIn",
  ...
  if (new URL(req.url).searchParams.get("format") === "csv") {
    const cols = ["firstName", "lastName", "email", "phone", "zip", "birthDate", "age",
    ...
    for (const r of rows) {
      lines.push([r.firstName, r.lastName, r.email, r.phone, r.zip, r.birthDate, ageOf(new Date(r.birthDate)),
