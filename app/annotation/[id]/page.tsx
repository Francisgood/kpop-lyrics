...
const STATUS = {
  approved: { label: "Approved & live", labelEs: "Aprobada y publicada", color: "#3ecf8e", bg: "rgba(62,207,142,0.12)" },
  rejected: { label: "Rejected by moderation", labelEs: "Rechazada por moderación", color: "#ff6b6b", bg: "rgba(255,107,107,0.12)" },
  ...
  return (
    <main>
      <div style={{ maxWidth: 720, margin: "0 auto", padding: "48px 24px 80px" }}>
        ...
        <div style={{ fontFamily: "var(--mono)", fontSize: "0.72rem", letterSpacing: "0.16em", textTransform: "uppercase", color: "var(--sakura)", marginBottom: 14 }}><T en="Annotation" es="Anotación" /></div>
        ...
        <div style={{ color: "var(--ink-dim)", fontSize: "0.95rem", marginBottom: 20 }}>
          ...
          <span style={{ marginLeft: 10, fontSize: "0.7rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", color: st.color, background: st.bg, padding: "2px 8px", borderRadius: 100 }}><T en={st.label} es={st.labelEs} /></span>
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14, background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 14, padding: "16px 18px" }}>
          ...
          <span style={{ flex: 1, minWidth: 0 }}>
            <span style={{ display: "block", fontSize: "0.7rem", textTransform: "uppercase", letterSpacing: "0.08em", color: "var(--ink-faint)" }}><T en="Contributed by" es="Contribución de" /></span>
          ...
          <Link href={`/u/${a.authorSlug}`} style={{ color: "var(--sakura)", fontWeight: 700, fontSize: "0.9rem", textDecoration: "none", flexShrink: 0 }}><T en="View profile →" es="Ver perfil →" /></Link>
        </div>

        {a.reviewedBy && (
          <div style={{ fontSize: "0.8rem", color: "var(--ink-faint)", marginTop: 16 }}>
            <T en={a.status === "approved" ? "Approved" : "Reviewed"} es={a.status === "approved" ? "Aprobada" : "Revisada"} />
            {" "}<T en="by" es="por" /> {a.reviewedBy}
            {a.reviewedAt ? (
              <T
                en={` on ${new Date(a.reviewedAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}`}
                es={` el ${new Date(a.reviewedAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}`}
              />
            ) : ""}.
          </div>
        )}
        ...
        <div style={{ marginTop: 28 }}>
          <Link href={`/u/${a.authorSlug}`} style={{ color: "var(--sakura)", fontWeight: 700, fontSize: "0.9rem", textDecoration: "none" }}>
            <T en={`← Back to ${a.authorName}’s profile`} es={`← Volver al perfil de ${a.authorName}`} />
          </Link>
        </div>
      </div>
    </main>
