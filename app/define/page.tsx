            ...
            return (
              <Link key={term.id} href={`/define/${term.slug}`} style={{ textDecoration: "none" }}>
                <div className="genius-card" style={{ padding: 20, height: "100%" }}>
                  <div style={{ fontWeight: 800, fontSize: "1.15rem", color: "var(--ink)", marginBottom: 8 }}>
                    {term.term}
                  </div>
                  {topDef && (
                    <div style={{ fontSize: "0.88rem", color: "var(--ink-dim)", lineHeight: 1.6, display: "-webkit-box", WebkitLineClamp: 3, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
                      {topDef.body}
                    </div>
                  )}
                  {topDef && (
