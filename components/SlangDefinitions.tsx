  ...
  return (
    <>
      <div style={{ background: "rgba(255,111,168,0.08)", border: "1px solid var(--sakura)", borderRadius: 8, padding: "10px 16px", marginBottom: 20, fontSize: "0.82rem", color: "var(--ink-dim)" }}>
        <strong style={{ color: "var(--sakura)" }}>Tip:</strong> vote on the definitions, or <strong style={{ color: "var(--sakura)" }}>highlight any text</strong> to add your own annotation.
      </div>
            ...
            <div key={def.id} className="genius-card" style={{ padding: 28, marginBottom: 20 }}>
              ...
              {def.example && (
                <div style={{ background: "rgba(255,111,168,0.18)", borderLeft: "3px solid var(--sakura)", padding: "12px 16px", borderRadius: "0 4px 4px 0", marginTop: 12 }}>
                  <div style={{ fontSize: "0.7rem", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--genius-gray)", marginBottom: 6 }}>Example</div>
      ...
      {annotate && (
        <div onClick={() => setAnnotate(null)} style={{ position: "fixed", inset: 0, zIndex: 300, background: "rgba(0,0,0,0.62)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ width: "min(440px, 94vw)", background: "var(--bg-card)", border: "1px solid var(--sakura)", borderRadius: 16, padding: "24px 22px", position: "relative" }}>
            <button type="button" onClick={() => setAnnotate(null)} aria-label="Close" style={{ position: "absolute", top: 12, right: 14, background: "none", border: "none", color: "var(--ink-faint)", fontSize: "1.5rem", lineHeight: 1, cursor: "pointer" }}>×</button>
            {annState === "done" ? (
              <div style={{ textAlign: "center", padding: "12px 0" }}>
                <div style={{ fontSize: "2rem", marginBottom: 8 }}>✓</div>
                <div style={{ color: "var(--ink)", fontWeight: 700 }}>Submitted for review</div>
                <div style={{ color: "var(--ink-dim)", fontSize: "0.85rem", marginTop: 4 }}>Thanks for contributing to {termName}!</div>
              </div>
            ) : (
              <>
                <div style={{ fontFamily: "var(--mono)", fontSize: "0.7rem", letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--sakura)", marginBottom: 8 }}>Annotate</div>
                <div style={{ fontSize: "0.9rem", color: "var(--ink-dim)", marginBottom: 12 }}>On <strong style={{ color: "var(--sakura)" }}>&ldquo;{annotate.text}&rdquo;</strong></div>
                <textarea value={note} onChange={(e) => setNote(e.target.value)} rows={4} autoFocus placeholder="Explain the nuance, origin, or usage…" style={{ width: "100%", background: "#fff", color: "#000", border: "1px solid var(--border-strong)", borderRadius: 8, padding: "10px 12px", fontSize: "0.9rem", boxSizing: "border-box", resize: "vertical" }} />
                <button type="button" onClick={submitAnnotation} disabled={annState === "saving" || !note.trim()} style={{ width: "100%", marginTop: 10, padding: "11px", borderRadius: 8, border: "none", background: "var(--sakura)", color: "var(--on-accent)", fontWeight: 700, cursor: "pointer", opacity: annState === "saving" || !note.trim() ? 0.6 : 1 }}>
                  {annState === "saving" ? "Submitting…" : "Submit annotation"}
                </button>
              </>
            )}
          </div>
